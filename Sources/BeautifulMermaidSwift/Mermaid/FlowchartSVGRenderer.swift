...
private func _renderSvgEntry(
    ...
    parts.append(CoreTheme.svgOpenTag(model.width, model.height, themeColors, transparent))
...
private func _renderGroup(_ group: _SvgGroup, _ font: String) -> String {
    _ = font
    let headerHeight = CoreStyles.FONT_SIZES.groupHeader + 16
    var parts: [String] = []
    ...
    parts.append(
        "  <rect x=\"\(group.x)\" y=\"\(group.y)\" width=\"\(group.width)\" height=\"\(group.height)\" " +
            "rx=\"0\" ry=\"0\" fill=\"var(--_group-fill)\" stroke=\"var(--_node-stroke)\" stroke-width=\"\(CoreStyles.STROKE_WIDTHS.outerBox)\" />"
    )
    parts.append(
        "  <rect x=\"\(group.x)\" y=\"\(group.y)\" width=\"\(group.width)\" height=\"\(headerHeight)\" " +
            "rx=\"0\" ry=\"0\" fill=\"var(--_group-hdr)\" stroke=\"var(--_node-stroke)\" stroke-width=\"\(CoreStyles.STROKE_WIDTHS.outerBox)\" />"
    )

    let header = MultilineText.renderMultilineText(
        group.label,
        cx: group.x + 12,
        cy: group.y + headerHeight / 2,
        fontSize: CoreStyles.FONT_SIZES.groupHeader,
        attrs: "font-size=\"\(CoreStyles.FONT_SIZES.groupHeader)\" font-weight=\"\(CoreStyles.FONT_WEIGHTS.groupHeader)\" fill=\"var(--_text-sec)\""
    )
    parts.append("  \(header)")
...
private func _renderEdge(_ edge: _SvgEdge) -> String {
    ...
    let baseStrokeWidth = edge.style == "thick"
        ? CoreStyles.STROKE_WIDTHS.connector * 2
        : CoreStyles.STROKE_WIDTHS.connector
...
private func _renderEdgeLabel(_ edge: _SvgEdge, _ font: String) -> String {
    ...
    let padding = 8.0
    let metrics = TextMetrics.measureMultilineText(
        label,
        fontSize: CoreStyles.FONT_SIZES.edgeLabel,
        fontWeight: CoreStyles.FONT_WEIGHTS.edgeLabel
    )

    let content = MultilineText.renderMultilineTextWithBackground(
        label,
        cx: mid.x,
        cy: mid.y,
        ...
        fontSize: CoreStyles.FONT_SIZES.edgeLabel,
        padding: padding,
        textAttrs: "text-anchor=\"middle\" font-size=\"\(CoreStyles.FONT_SIZES.edgeLabel)\" font-weight=\"\(CoreStyles.FONT_WEIGHTS.edgeLabel)\" fill=\"var(--_text-sec)\"",
...
private func _renderNodeShape(_ node: _SvgNode) -> String {
    ...
    let sw = _escapeAttr(inlineStyle["stroke-width"] ?? "\(CoreStyles.STROKE_WIDTHS.innerBox)")
...
private func _renderStateEnd(x: Double, y: Double, w: Double, h: Double) -> String {
    ...
    let innerR = outerR - 4
    return "<circle cx=\"\(cx)\" cy=\"\(cy)\" r=\"\(outerR)\" fill=\"none\" stroke=\"var(--_text)\" stroke-width=\"\(CoreStyles.STROKE_WIDTHS.innerBox * 2)\" />\n" +
...
private func _renderNodeLabel(_ node: _SvgNode, _ font: String) -> String {
    ...
    return MultilineText.renderMultilineText(
        node.label,
        cx: cx,
        cy: cy,
        fontSize: CoreStyles.FONT_SIZES.nodeLabel,
        attrs: "text-anchor=\"middle\" font-size=\"\(CoreStyles.FONT_SIZES.nodeLabel)\" font-weight=\"\(CoreStyles.FONT_WEIGHTS.nodeLabel)\" fill=\"\(textColor)\""
    )
}
...
private func _extractSvgGraphModel(_ graph: PositionedGraph) -> _SvgGraphModel {
    _SvgGraphModel(
        width: graph.width,
        height: graph.height,
        nodes: (graph.flowchartNodes ?? []).map(_extractNode),
        edges: (graph.flowchartEdges ?? []).map(_extractEdge),
        groups: (graph.flowchartGroups ?? []).map(_extractGroup)
    )
}

// Direct typed extraction. This used to reflect over every field of every
// element with `Mirror` (a linear children scan per field) — ~19% of the SVG
// stage in profiles. `String(describing:)` on the two enums reproduces the
// reflection path's exact output.
private func _extractNode(_ node: PositionedNode) -> _SvgNode {
    _SvgNode(
        id: node.id,
        label: node.label,
...
private func _extractEdge(_ edge: PositionedEdge) -> _SvgEdge {
    _SvgEdge(
        source: edge.source,
        target: edge.target,
        label: edge.label,
        style: String(describing: edge.style),
        hasArrowStart: edge.hasArrowStart,
        hasArrowEnd: edge.hasArrowEnd,
        points: edge.points.map(_extractPoint),
        labelPosition: edge.labelPosition.map(_extractPoint),
        inlineStyle: edge.inlineStyle
    )
}

private func _extractGroup(_ group: PositionedGroup) -> _SvgGroup {
    _SvgGroup(
        id: group.id,
        label: group.label,
        x: group.x,
        ...
        width: group.width,
        height: group.height,
        children: group.children.map(_extractGroup)
    )
}

private func _extractPoint(_ point: _PositionedPointPayload) -> _SvgPoint {
    _SvgPoint(x: point.x, y: point.y)
}
