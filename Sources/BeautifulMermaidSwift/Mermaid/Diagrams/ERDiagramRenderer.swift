...
private func _renderErSvgEntry(
    ...
    parts.append(CoreTheme.buildStyleBlock(font, true))
    parts.append("<defs>")
    parts.append("</defs>")
...
private func _toThemeColors(_ colors: DiagramColors) -> CoreTheme.DiagramColors {
    CoreTheme.DiagramColors(
        bg: colors.bg,
        fg: colors.fg,
        line: colors.line,
...
private func _renderEntityBox(_ entity: PositionedErEntity) -> String {
    ...
    parts.append(
        "  <rect x=\"\(x)\" y=\"\(y)\" width=\"\(width)\" height=\"\(height)\" " +
            ...
            "stroke-width=\"\(CoreStyles.STROKE_WIDTHS.outerBox)\" />"
    )
    ...
    parts.append(
        "  " + MultilineText.renderMultilineText(
            ...
            attrs: "text-anchor=\"middle\" font-size=\"\(CoreStyles.FONT_SIZES.nodeLabel)\" " +
    ...
    parts.append(
        "  <line x1=\"\(x)\" y1=\"\(attrTop)\" x2=\"\(x + width)\" y2=\"\(attrTop)\" " +
            "stroke=\"var(--_node-stroke)\" stroke-width=\"\(CoreStyles.STROKE_WIDTHS.innerBox)\" />"
    )
    ...
    if attrs.isEmpty {
        parts.append(
            "  <text x=\"\(x + width / 2)\" y=\"\(attrTop + rowHeight / 2)\" text-anchor=\"middle\" " +
                "dy=\"\(CoreStyles.TEXT_BASELINE_SHIFT)\" font-size=\"\(ERFont.attrSize)\" " +
...
private func _renderAttribute(
    ...
    if hasComment {
        ...
        parts.append("<g><title>\(MultilineText.escapeXml(tooltipText))</title>")
    }

    var keyWidth = 0.0
    if !attr.keys.isEmpty {
        let keyText = attr.keys.joined(separator: ",")
        keyWidth = CoreStyles.estimateTextWidth(keyText, ERFont.keySize, ERFont.keyWeight) + 8
        parts.append(
        ...
        parts.append(
            "<text x=\"\(boxX + 6 + keyWidth / 2)\" y=\"\(y)\" text-anchor=\"middle\" " +
                "dy=\"\(CoreStyles.TEXT_BASELINE_SHIFT)\" font-size=\"\(ERFont.keySize)\" " +
    ...
    parts.append(
        "<text x=\"\(typeX)\" y=\"\(y)\" class=\"mono\" dy=\"\(CoreStyles.TEXT_BASELINE_SHIFT)\" " +
            "font-size=\"\(ERFont.attrSize)\" font-weight=\"\(ERFont.attrWeight)\">" +
            "<tspan fill=\"var(--_text-muted)\">\(MultilineText.escapeXml(attr.type))</tspan></text>"
    )

    let nameX = boxX + boxWidth - 8
    parts.append(
        "<text x=\"\(nameX)\" y=\"\(y)\" class=\"mono\" text-anchor=\"end\" dy=\"\(CoreStyles.TEXT_BASELINE_SHIFT)\" " +
            "font-size=\"\(ERFont.attrSize)\" font-weight=\"\(ERFont.attrWeight)\">" +
            "<tspan fill=\"var(--_text-sec)\">\(MultilineText.escapeXml(attr.name))</tspan></text>"
    )
...
private func _renderRelationshipLine(_ rel: PositionedErRelationship) -> String {
    ...
    return "<polyline \(attrs)\(labelAttr) points=\"\(pathData)\" fill=\"none\" stroke=\"var(--_line)\" " +
        "stroke-width=\"\(CoreStyles.STROKE_WIDTHS.connector)\"\(dashArray) />"
}
...
private func _renderRelationshipLabel(_ rel: PositionedErRelationship) -> String {
    ...
    return "<rect x=\"\(mid.x - bgW / 2)\" y=\"\(mid.y - bgH / 2)\" width=\"\(bgW)\" height=\"\(bgH)\" rx=\"2\" ry=\"2\" " +
        ...
        MultilineText.renderMultilineText(
            ...
            cy: mid.y,
            fontSize: CoreStyles.FONT_SIZES.edgeLabel,
            attrs: "text-anchor=\"middle\" font-size=\"\(CoreStyles.FONT_SIZES.edgeLabel)\" " +
                "font-weight=\"\(CoreStyles.FONT_WEIGHTS.edgeLabel)\" fill=\"var(--_text-muted)\""
        )
}
...
private func _renderCrowsFoot(point: ErPoint, toward: ErPoint, cardinality: Cardinality) -> String {
    let sw = CoreStyles.STROKE_WIDTHS.connector + 0.25
