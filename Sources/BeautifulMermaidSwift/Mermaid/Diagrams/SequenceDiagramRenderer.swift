...
private func _renderSequenceSvgEntry(
    ...
    var parts: [String] = []
    let themeColors = CoreTheme.DiagramColors(
        bg: colors.bg,
        fg: colors.fg,
        line: colors.line,
    ...
    parts.append(CoreTheme.svgOpenTag(diagram.width, diagram.height, themeColors, transparent))
    parts.append(CoreTheme.buildStyleBlock(font, false))
    parts.append("<defs>")
...
private func arrowMarkerDefs() -> String {
    let w = CoreStyles.ARROW_HEAD.width
    let h = CoreStyles.ARROW_HEAD.height
...
private func renderActor(_ actor: PositionedSequenceActor) -> String {
    ...
    if type == "actor" {
        ...
        let sw = CoreStyles.STROKE_WIDTHS.outerBox / s
        ...
        parts.append(
            "  " + MultilineText.renderMultilineText(
                ...
                cx: x,
                cy: y + height + 14,
                fontSize: CoreStyles.FONT_SIZES.nodeLabel,
                attrs: "font-size=\"\(CoreStyles.FONT_SIZES.nodeLabel)\" text-anchor=\"middle\" font-weight=\"\(CoreStyles.FONT_WEIGHTS.nodeLabel)\" fill=\"var(--_text)\""
            )
        )
    } else {
        let boxX = x - width / 2
        parts.append(
            "  <rect x=\"\(boxX)\" y=\"\(y)\" width=\"\(width)\" height=\"\(height)\" rx=\"4\" ry=\"4\" fill=\"var(--_node-fill)\" stroke=\"var(--_node-stroke)\" stroke-width=\"\(CoreStyles.STROKE_WIDTHS.outerBox)\" />"
        )
        parts.append(
            "  " + MultilineText.renderMultilineText(
                label,
...
private func renderActivation(_ activation: SequenceActivation) -> String {
    "<rect class=\"activation\" data-actor=\"\(escapeAttr(activation.actorId))\" x=\"\(activation.x)\" y=\"\(activation.topY)\" width=\"\(activation.width)\" height=\"\(activation.bottomY - activation.topY)\" fill=\"var(--_node-fill)\" stroke=\"var(--_node-stroke)\" stroke-width=\"\(CoreStyles.STROKE_WIDTHS.innerBox)\" />"
}
...
private func renderMessage(_ msg: PositionedSequenceMessage) -> String {
    ...
    if msg.isSelf {
        ...
        parts.append(
            "  <polyline points=\"\(msg.x1),\(msg.y) \(msg.x1 + loopW),\(msg.y) \(msg.x1 + loopW),\(msg.y + loopH) \(msg.x2),\(msg.y + loopH)\" fill=\"none\" stroke=\"var(--_line)\" stroke-width=\"\(CoreStyles.STROKE_WIDTHS.connector)\"\(dashArray) marker-end=\"url(#\(markerId))\" />"
        )
        parts.append(
            "  " + MultilineText.renderMultilineText(
                msg.label,
                ...
                cy: msg.y + loopH / 2,
                fontSize: CoreStyles.FONT_SIZES.edgeLabel,
                attrs: "font-size=\"\(CoreStyles.FONT_SIZES.edgeLabel)\" text-anchor=\"start\" font-weight=\"\(CoreStyles.FONT_WEIGHTS.edgeLabel)\" fill=\"var(--_text-muted)\""
            )
        )
    } else {
        parts.append(
            "  <line x1=\"\(msg.x1)\" y1=\"\(msg.y)\" x2=\"\(msg.x2)\" y2=\"\(msg.y)\" stroke=\"var(--_line)\" stroke-width=\"\(CoreStyles.STROKE_WIDTHS.connector)\"\(dashArray) marker-end=\"url(#\(markerId))\" />"
        )
        ...
        parts.append(
            "  " + MultilineText.renderMultilineText(
                ...
                cy: msg.y - 6,
                fontSize: CoreStyles.FONT_SIZES.edgeLabel,
                attrs: "font-size=\"\(CoreStyles.FONT_SIZES.edgeLabel)\" text-anchor=\"middle\" font-weight=\"\(CoreStyles.FONT_WEIGHTS.edgeLabel)\" fill=\"var(--_text-muted)\""
            )
        )
    }
...
private func renderBlock(_ block: PositionedSequenceBlock) -> String {
    ...
    parts.append(
        "  <rect x=\"\(block.x)\" y=\"\(block.y)\" width=\"\(block.width)\" height=\"\(block.height)\" rx=\"0\" ry=\"0\" fill=\"none\" stroke=\"var(--_node-stroke)\" stroke-width=\"\(CoreStyles.STROKE_WIDTHS.outerBox)\" />"
    )

    let labelText = block.label.isEmpty ? block.type : "\(block.type) [\(block.label)]"
    let firstLine = labelText.components(separatedBy: "\n").first ?? labelText
    let tabWidth = CoreStyles.estimateTextWidth(
        firstLine,
        CoreStyles.FONT_SIZES.edgeLabel,
        CoreStyles.FONT_WEIGHTS.groupHeader
    ) + 16
    let tabHeight = 18.0

    parts.append(
        "  <rect x=\"\(block.x)\" y=\"\(block.y)\" width=\"\(tabWidth)\" height=\"\(tabHeight)\" fill=\"var(--_group-hdr)\" stroke=\"var(--_node-stroke)\" stroke-width=\"\(CoreStyles.STROKE_WIDTHS.outerBox)\" />"
    )
    ...
    parts.append(
        "  " + MultilineText.renderMultilineText(
            ...
            fontSize: CoreStyles.FONT_SIZES.edgeLabel,
            attrs: "font-size=\"\(CoreStyles.FONT_SIZES.edgeLabel)\" font-weight=\"\(CoreStyles.FONT_WEIGHTS.groupHeader)\" fill=\"var(--_text-sec)\""
        )
    )
...
private func renderNote(_ note: PositionedSequenceNote) -> String {
    ...
    let noteTextAttrs =
        "font-size=\"\(CoreStyles.FONT_SIZES.edgeLabel)\" "
        + "text-anchor=\"middle\" "
        + "font-weight=\"\(CoreStyles.FONT_WEIGHTS.edgeLabel)\" "
        + "fill=\"var(--_text-muted)\""
    let noteText = MultilineText.renderMultilineText(
        note.text,
    ...
    return "<g class=\"note\"\(positionAttr)\(actorsAttr)>"
        + "\n  <rect x=\"\(note.x)\" y=\"\(note.y)\" width=\"\(note.width)\" height=\"\(note.height)\" fill=\"var(--_group-hdr)\" stroke=\"var(--_node-stroke)\" stroke-width=\"\(CoreStyles.STROKE_WIDTHS.innerBox)\" />"
...
public enum SequenceDiagramRenderer {
