...
private func buildClassElkGraph(
...
) -> (elkGraph: GraphJSON, classSizes: ClassSizeMap) {
    _ = options
    ...
    for cls in diagram.classes {
        ...
        let headerTextW = CoreStyles.estimateTextWidth(
            cls.label,
...
private func _layoutClassDiagramSyncEntry(
    ...
    let result = try runGraphLayout(built.elkGraph)
...
private func maxMemberWidth(_ members: [ClassMember]) -> Double {
    ...
    for member in members {
        ...
        let w = CoreStyles.estimateMonoTextWidth(text, CLS.memberFontSize)
        if w > maxW {
            maxW = w
        }
    }
...
public enum ClassDiagramLayout {
