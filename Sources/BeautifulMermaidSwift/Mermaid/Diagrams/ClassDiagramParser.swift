...
private func _parseRelationship(_ line: String) -> ClassRelationship? {
    ...
    let fromCardinality = groups[safe: 2].flatMap { $0.isEmpty ? nil : MultilineText.normalizeBrTags($0) }
    let toCardinality = groups[safe: 4].flatMap { $0.isEmpty ? nil : MultilineText.normalizeBrTags($0) }
    let label = groups[safe: 6].flatMap {
        let value = $0.trimmingCharacters(in: .whitespacesAndNewlines)
        return value.isEmpty ? nil : MultilineText.normalizeBrTags(value)
    }
...
public enum ClassDiagramParser {
