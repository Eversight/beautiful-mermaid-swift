...
public func renderXYChartSvg(
    ...
    let themeColors = CoreTheme.DiagramColors(bg: colors.bg, fg: colors.fg, line: colors.line, accent: colors.accent, muted: colors.muted, surface: colors.surface, border: colors.border)
    var svgTag = CoreTheme.svgOpenTag(chart.width, chart.height, themeColors, transparent)
    ...
    if interactive || sparse {
        ...
        // Iterate columns left-to-right by x. `columns` is a Dictionary, whose
        // `.values` order is randomized per process — iterating it directly makes
        // the emitted dot/overlay order non-deterministic across runs (identical
        // picture, different bytes). Sorting by x makes SVG output reproducible.
        let orderedColumns = columns.values.sorted { ($0.first?.x ?? 0) < ($1.first?.x ?? 0) }
        for entries in orderedColumns {
    ...
    // 5. Axis labels
    let TEXT_BASELINE = CoreStyles.TEXT_BASELINE_SHIFT
...
private func _tooltipAbove(_ cx: Double, _ topY: Double, _ text: String) -> String {
    let textW = CoreStyles.estimateTextWidth(text, TIP.fontSize, TIP.fontWeight)
    ...
    return "<rect x=\"\(_r(bgX))\" y=\"\(_r(tipY))\" width=\"\(_r(bgW))\" height=\"\(bgH)\" rx=\"\(TIP.rx)\" class=\"xychart-tip xychart-tip-bg\"/>" +
        pointer +
        "<text x=\"\(_r(textX))\" y=\"\(_r(textY))\" text-anchor=\"middle\" dy=\"\(CoreStyles.TEXT_BASELINE_SHIFT)\" class=\"xychart-tip xychart-tip-text\">\(_escapeXml(text))</text>"
}
...
private func _multiTooltipAbove(_ cx: Double, _ topY: Double, _ label: String, _ entries: [(text: String, legendLabel: String)]) -> String {
    ...
    let headingW = CoreStyles.estimateTextWidth(label, TIP.fontSize, 600)
    let maxRowW = entries.map { e in
        CoreStyles.estimateTextWidth(e.legendLabel, TIP.fontSize, TIP.fontWeight) + labelGap +
        CoreStyles.estimateTextWidth(e.text, TIP.fontSize, TIP.fontWeight)
    }.max() ?? 0
    ...
    svg += "<text x=\"\(_r(cx))\" y=\"\(_r(textY))\" text-anchor=\"middle\" font-weight=\"600\" font-size=\"\(TIP.fontSize)\" dy=\"\(CoreStyles.TEXT_BASELINE_SHIFT)\" class=\"xychart-tip xychart-tip-text\">\(_escapeXml(label))</text>"

    let rowLeft = bgX + TIP.padX
    let rowRight = bgX + bgW - TIP.padX
    for entry in entries {
        textY += lineH
        svg += "<text x=\"\(_r(rowLeft))\" y=\"\(_r(textY))\" text-anchor=\"start\" font-size=\"\(TIP.fontSize)\" font-weight=\"\(TIP.fontWeight)\" dy=\"\(CoreStyles.TEXT_BASELINE_SHIFT)\" class=\"xychart-tip xychart-tip-text\">\(_escapeXml(entry.legendLabel))</text>"
        svg += "<text x=\"\(_r(rowRight))\" y=\"\(_r(textY))\" text-anchor=\"end\" font-size=\"\(TIP.fontSize)\" font-weight=\"\(TIP.fontWeight)\" dy=\"\(CoreStyles.TEXT_BASELINE_SHIFT)\" class=\"xychart-tip xychart-tip-text\">\(_escapeXml(entry.text))</text>"
    }
