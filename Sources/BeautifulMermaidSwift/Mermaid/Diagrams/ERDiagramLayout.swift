...
) -> (elkGraph: _GraphJSON, entitySizes: EntitySizeMap) {
    _ = options
    var entitySizes: EntitySizeMap = [:]

    for entity in diagram.entities {
        let headerTextWidth = CoreStyles.estimateTextWidth(
            entity.label,
            CoreStyles.FONT_SIZES.nodeLabel,
            CoreStyles.FONT_WEIGHTS.nodeLabel
        )
        ...
        for attr in entity.attributes {
            ...
            let width = CoreStyles.estimateMonoTextWidth(attrText, ER.attrFontSize)
...
private func _decodeElkNode(_ any: Any?) -> _GraphJSON? {
...
private func _encodeElkNode(_ node: _GraphJSON) -> GraphJSON {
    var out: GraphJSON = ["id": node.id]
...
private func _runGraphLayout(_ graph: _GraphJSON) throws -> _GraphJSON {
    let laidOut = try runGraphLayout(_encodeElkNode(graph))
...
private func _layoutErDiagramSyncEntry(
    ...
    let result = try _runGraphLayout(built.elkGraph)
    return _extractErLayout(result, diagram, built.entitySizes)
}

public enum ERDiagramLayout {
