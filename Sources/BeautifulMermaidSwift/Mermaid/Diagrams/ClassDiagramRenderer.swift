...
private func _renderClassSvgEntry(
    ...
    let themedColors = CoreTheme.DiagramColors(
        bg: colors.bg,
        fg: colors.fg,
        line: colors.line,
    ...
    parts.append(CoreTheme.svgOpenTag(diagram.width, diagram.height, themedColors, transparent))
...
private func _renderClassBox(_ cls: PositionedClassNode) -> String {
    ...
    parts.append(
        "  <rect x=\"\(x)\" y=\"\(y)\" width=\"\(width)\" height=\"\(height)\" " +
            "rx=\"0\" ry=\"0\" fill=\"var(--_node-fill)\" stroke=\"var(--_node-stroke)\" stroke-width=\"\(CoreStyles.STROKE_WIDTHS.outerBox)\" />"
    )
    ...
    if let annotation = cls.annotation {
        let annotY = y + 12
        parts.append(
            "  <text x=\"\(x + width / 2)\" y=\"\(annotY)\" text-anchor=\"middle\" dy=\"\(CoreStyles.TEXT_BASELINE_SHIFT)\" " +
                "font-size=\"\(_ClassFont.annotationSize)\" font-weight=\"\(_ClassFont.annotationWeight)\" " +
                "font-style=\"italic\" fill=\"var(--_text-muted)\">&lt;&lt;\(MultilineText.escapeXml(annotation))&gt;&gt;</text>"
        )
    ...
    parts.append(
        "  " + MultilineText.renderMultilineText(
            ...
            attrs: "text-anchor=\"middle\" font-size=\"\(CoreStyles.FONT_SIZES.nodeLabel)\" font-weight=\"700\" fill=\"var(--_text)\""
        )
    )
...
private func _renderMember(_ member: ClassMember, _ x: Double, _ y: Double) -> String {
    ...
    if !member.visibility.isEmpty {
        spans.append("<tspan fill=\"var(--_text-faint)\">\(MultilineText.escapeXml(member.visibility)) </tspan>")
    }

    let displayName = member.isMethod
        ? "\(member.name)(\(member.params ?? ""))"
        : member.name
    spans.append("<tspan fill=\"var(--_text-sec)\">\(MultilineText.escapeXml(displayName))</tspan>")

    if let type = member.type, !type.isEmpty {
        spans.append("<tspan fill=\"var(--_text-faint)\">: </tspan>")
        spans.append("<tspan fill=\"var(--_text-muted)\">\(MultilineText.escapeXml(type))</tspan>")
    }

    return "<text x=\"\(x)\" y=\"\(y)\" class=\"mono\" dy=\"\(CoreStyles.TEXT_BASELINE_SHIFT)\" " +
...
private func _renderRelationship(_ rel: PositionedClassRelationship) -> String {
    ...
    return "<polyline \(dataAttrs.joined(separator: " ")) points=\"\(pathData)\" fill=\"none\" stroke=\"var(--_line)\" " +
        "stroke-width=\"\(CoreStyles.STROKE_WIDTHS.connector)\"\(dashArray)\(markers) />"
}
...
public enum ClassDiagramRenderer {
