...
private func _parseAttribute(_ line: String) -> ErAttribute? {
    ...
    if let commentMatch = _firstGroup(#""([^"]*)""#, rest) {
        comment = MultilineText.normalizeBrTags(commentMatch)
    }
...
private func _parseRelationshipLine(_ line: String) -> ErRelationship? {
    ...
    let label = MultilineText.normalizeBrTags(rawLabel.trimmingCharacters(in: .whitespacesAndNewlines).replacingOccurrences(of: #"^["']|["']$"#, with: "", options: .regularExpression))
