...
/// A layout graph in dictionary form (nodes carry `children`/`edges` arrays),
/// as consumed and produced by the layout bridge.
public typealias GraphJSON = [String: Any]

@available(*, deprecated, renamed: "GraphJSON")
...
private enum _ElkBridgeRuntime {
    private static let sharedInstance = Mutex<LayoutEngine?>(nil)

    static func shared() throws -> LayoutEngine {
        try sharedInstance.withLock { instance in
            if let instance {
                return instance
            }
            do {
                let created = try LayoutEngine()
                instance = created
                return created
            } catch {
...
private func _validateElkGraph(_ graph: GraphJSON) throws {
...
private func _parseDirection(_ graph: GraphJSON) -> _ElkDirection {
    guard
...
private func _parsePadding(_ graph: GraphJSON) -> _ElkPadding {
    guard
        let layoutOptions = graph["layoutOptions"] as? [String: Any],
...
private func _parseSpacing(_ graph: GraphJSON) -> (nodeSpacing: Double, layerSpacing: Double) {
...
private func _extractNodeDimensions(_ node: GraphJSON) -> (id: String, width: Double, height: Double)? {
...
private func _point(_ x: Double, _ y: Double) -> GraphJSON {
    ["x": x, "y": y]
}

private func _routeEdges(edges: inout [GraphJSON], nodesById: [String: _ElkLaidOutNode]) {
    for idx in edges.indices {
        ...
        if var labels = edges[idx]["labels"] as? [GraphJSON], !labels.isEmpty {
            var first = labels[0]
...
private func _layoutRecursively(_ graph: GraphJSON) throws -> GraphJSON {
    var out = graph
    var children = (out["children"] as? [GraphJSON]) ?? []

    // Layout nested compounds first.
    if !children.isEmpty {
        for i in children.indices {
            if children[i]["children"] is [GraphJSON] {
                children[i] = try _layoutRecursively(children[i])
            }
        }
    }

    var edges = (out["edges"] as? [GraphJSON]) ?? []
...
public func runGraphLayout(_ graph: GraphJSON) throws -> GraphJSON {
    try LayoutBridge.layout(graph)
}

@available(*, deprecated, renamed: "runGraphLayout(_:)")
public func elkLayoutSync(_ graph: GraphJSON) throws -> GraphJSON {
    try LayoutBridge.layout(graph)
}

@available(*, deprecated, renamed: "LayoutBridge")
public typealias ElkAdapter = LayoutBridge

public enum LayoutBridge {

    // Export inventory from TypeScript source:
    // - export function elkLayoutSync
    public static func layout(_ graph: GraphJSON) throws -> GraphJSON {
