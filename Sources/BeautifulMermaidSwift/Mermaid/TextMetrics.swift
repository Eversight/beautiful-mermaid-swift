...
public enum TextMetrics {
    ...
    public static func getCharWidth(_ char: Character) -> Double {
        ...
        // Fast path: a Character whose first scalar is ASCII (< 0x80) is never
        // fullwidth and never an emoji sequence (emoji graphemes begin with a
        // pictographic base). Gating here skips the per-character emoji regex —
        // which ran an NSRegularExpression match on every char of every label —
        // for the overwhelmingly common ASCII case, with identical results.
        if code >= 0x80, isFullwidth(code) || isEmoji(char) {
            return 2.0
        }
        if char == " " {
            return 0.3
    ...
    public static func measureMultilineText(
        ...
        var maxWidth = 0.0
        for line in lines {
            // stripFormattingTags is a no-op when there are no tags, so skip the
            // regex scan entirely for the common tag-free line — identical result.
            let plain = line.contains("<") ? stripFormattingTags(line) : line
    ...
    private static let FORMATTING_TAG_REGEX: NSRegularExpression = {
        guard let regex = try? NSRegularExpression(pattern: #"</?(?:b|strong|i|em|u|s|del)\s*>"#, options: [.caseInsensitive]) else {
            assertionFailure("Invalid formatting-tag regex")
            return NSRegularExpression()
        }
        return regex
    }()
    ...
    private static func stripFormattingTags(_ text: String) -> String {
        let range = NSRange(text.startIndex..<text.endIndex, in: text)
        return FORMATTING_TAG_REGEX.stringByReplacingMatches(in: text, options: [], range: range, withTemplate: "")
    }
}
