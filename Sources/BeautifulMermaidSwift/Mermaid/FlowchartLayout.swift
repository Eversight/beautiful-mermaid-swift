...
private typealias _ParsedGraph = CoreTypes.MermaidGraph
private typealias _ParsedNode = CoreTypes.MermaidNode
private typealias _ParsedEdge = CoreTypes.MermaidEdge

private typealias _GraphJSON = [String: Any]
...
private func _mapDirection(_ direction: CoreTypes.Direction) -> String {
    switch direction {
...
private func _nodeSize(_ node: _ParsedNode) -> (width: Double, height: Double) {
    let metrics = TextMetrics.measureMultilineText(
        ...
        fontWeight: CoreStyles.FONT_WEIGHTS.nodeLabel
    )
...
private func _buildElkGraph(_ graph: _ParsedGraph) -> _GraphJSON {
    ...
    func _edgeDict(_ idx: Int, _ edge: CoreTypes.MermaidEdge) -> [String: Any] {
        var out: [String: Any] = [
            "id": "e\(idx)",
            "sources": [edge.source],
            "targets": [edge.target]
        ...
        if let label = edge.label, !label.isEmpty {
            let m = TextMetrics.measureMultilineText(
                label,
    ...
    func _deepestSubgraph(for nodeId: String, in subs: [CoreTypes.MermaidSubgraph]) -> String? {
        for sub in subs {
    ...
    struct CrossEdge {
        let idx: Int
        let edge: CoreTypes.MermaidEdge
        let srcSub: String?
        let tgtSub: String?
    }
    ...
    for ce in crossEdges {
        ...
        if let srcSg = ce.srcSub {
            ...
            if let label = ce.edge.label, !label.isEmpty {
                let m = TextMetrics.measureMultilineText(
                    label, fontSize: CoreStyles.FONT_SIZES.edgeLabel,
                    fontWeight: CoreStyles.FONT_WEIGHTS.edgeLabel)
    ...
    func buildSubgraphNode(_ sub: CoreTypes.MermaidSubgraph) -> [String: Any] {
...
private func _buildSubgraphOwnership(
    _ subs: [CoreTypes.MermaidSubgraph]
) -> [String: Set<String>] {
...
private func _allNodeIds(in sub: CoreTypes.MermaidSubgraph) -> Set<String> {
...
private func _subgraphContainsNode(_ sub: CoreTypes.MermaidSubgraph, nodeId: String) -> Bool {
...
private func _collectAllChildren(
    ...
    nodeById: [String: CoreTypes.MermaidNode],
...
private func _alignLayerNodes(
    ...
    _ direction: CoreTypes.Direction
) {
...
private func _allSubgraphIds(_ subs: [CoreTypes.MermaidSubgraph]) -> [String] {
    subs.flatMap { [$0.id] + _allSubgraphIds($0.children) }
}

private func _findSubgraphLabel(_ id: String, in subs: [CoreTypes.MermaidSubgraph]) -> String? {
    for sub in subs {
...
private func _layoutGraphSyncWithConfig(
    ...
    do {
        let laidOut = try runGraphLayout(elkGraph)
        ...
        let laidOut = try runGraphLayout(flatGraph)
        return _extractPositionedGraph(parsed, laidOut, diagramType: graph.type)
    }
}

/// Patch ELK layout options on a built graph with LayoutConfig values.
private func _applyLayoutConfig(_ config: LayoutConfig, to elkGraph: inout _GraphJSON) {
...
private func _buildElkGraphNoCrossEdges(_ graph: _ParsedGraph) -> _GraphJSON {
    let subgraphOwnership = _buildSubgraphOwnership(graph.subgraphs)
    let allClaimedNodes = Set(subgraphOwnership.values.flatMap { $0 })
    let nodeById = Dictionary(graph.nodesInOrder.map { ($0.id, $0.node) }, uniquingKeysWith: { _, last in last })
    ...
    for (idx, edge) in graph.edges.enumerated() {
        ...
        if let label = edge.label, !label.isEmpty {
            let m = TextMetrics.measureMultilineText(label, fontSize: CoreStyles.FONT_SIZES.edgeLabel, fontWeight: CoreStyles.FONT_WEIGHTS.edgeLabel)
...
private func _buildFlatElkGraph(_ graph: _ParsedGraph) -> _GraphJSON {
    var children: [[String: Any]] = []
    for entry in graph.nodesInOrder {
        let size = _nodeSize(entry.node)
        children.append([
...
private func _edgePathMidpoint(_ points: [_PositionedPointPayload], direction: CoreTypes.Direction = .TD) -> _PositionedPointPayload {
