...
public enum AsciiGrid {

    public typealias GridCoord = AsciiConverter.GridCoord
    public typealias DrawingCoord = AsciiConverter.DrawingCoord
    public typealias AsciiGraph = AsciiConverter.AsciiGraph
    public typealias AsciiNode = AsciiConverter.AsciiNode
    public typealias AsciiSubgraph = AsciiConverter.AsciiSubgraph

    public static func gridToDrawingCoord(
        _ graph: AsciiGraph,
        _ c: GridCoord,
        dir: AsciiConverter.Direction? = nil
    ) -> DrawingCoord {
        ...
        while placedCount < graph.nodes.count {
            ...
            for node in graph.nodes {
                ...
                for child in getChildren(graph, node) where child.gridCoord == nil {
                    ...
                    // `childLevel` grows with graph depth (and, via collision
                    // resolution + subgraph direction overrides, with grid
                    // position), so it is not bounded by any fixed constant.
                    // Grow the level table on demand instead of pre-sizing it,
                    // which previously crashed for diagrams deeper than 25 levels.
                    if childLevel >= highestPositionPerLevel.count {
                        highestPositionPerLevel.append(
                            contentsOf: repeatElement(0, count: childLevel - highestPositionPerLevel.count + 1)
                        )
                    }
    ...
    private static let Up = AsciiConverter.Direction(x: 1, y: 0)
    private static let Down = AsciiConverter.Direction(x: 1, y: 2)
    private static let Left = AsciiConverter.Direction(x: 0, y: 1)
    private static let Right = AsciiConverter.Direction(x: 2, y: 1)
    private static let UpperRight = AsciiConverter.Direction(x: 2, y: 0)
    private static let UpperLeft = AsciiConverter.Direction(x: 0, y: 0)
    private static let LowerRight = AsciiConverter.Direction(x: 2, y: 2)
    private static let LowerLeft = AsciiConverter.Direction(x: 0, y: 2)
    private static let Middle = AsciiConverter.Direction(x: 1, y: 1)
    ...
    private static func dirEquals(
        _ a: AsciiConverter.Direction,
        _ b: AsciiConverter.Direction
    ) -> Bool {
        a.x == b.x && a.y == b.y
    }

    private static func getOpposite(
        _ d: AsciiConverter.Direction
    ) -> AsciiConverter.Direction {
    ...
    private static func gridCoordDirection(
        _ c: GridCoord,
        _ dir: AsciiConverter.Direction
    ) -> GridCoord {
        GridCoord(x: c.x + dir.x, y: c.y + dir.y)
    }

    private static func determineDirection(from: GridCoord, to: GridCoord) -> AsciiConverter.Direction {
        if from.x == to.x {
    ...
    private static func determineStartAndEndDir(
        _ edge: AsciiConverter.AsciiEdge,
        _ graphDirection: String
    ) -> (
        AsciiConverter.Direction,
        AsciiConverter.Direction,
    ...
    private static func _determinePath(_ graph: AsciiGraph, _ edge: AsciiConverter.AsciiEdge) {
        let sourceSg = getNodeSubgraph(graph, edge.from)
        ...
        let preferredPathRaw = AsciiPathfinder.getPath(graph.grid, from: prefFrom, to: prefTo)
        let alternativePathRaw = AsciiPathfinder.getPath(graph.grid, from: altFrom, to: altTo)

        if let preferredPathRaw, let alternativePathRaw {
            let preferredPath = AsciiPathfinder.mergePath(preferredPathRaw)
            let alternativePath = AsciiPathfinder.mergePath(alternativePathRaw)
        ...
        if let preferredPathRaw {
            edge.startDir = preferredDir
            edge.endDir = preferredOppositeDir
            edge.path = AsciiPathfinder.mergePath(preferredPathRaw)
            return
        }
        if let alternativePathRaw {
            edge.startDir = alternativeDir
            edge.endDir = alternativeOppositeDir
            edge.path = AsciiPathfinder.mergePath(alternativePathRaw)
            return
        }
    ...
    private static func _determineLabelLine(_ graph: AsciiGraph, _ edge: AsciiConverter.AsciiEdge) {
    ...
    private static func _setRoleCanvasSizeToGrid(_ graph: AsciiGraph) {
        ...
        graph.roleCanvas = (0 ..< maxX).map { _ in
            Array(repeating: AsciiConverter.CharRole.none, count: maxY)
        }
    }
    ...
    private static func getEdgesFromNode(_ graph: AsciiGraph, _ node: AsciiNode) -> [AsciiConverter.AsciiEdge] {
