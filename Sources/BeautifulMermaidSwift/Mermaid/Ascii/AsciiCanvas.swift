...
public typealias AsciiNodeShape = AsciiTypes.AsciiNodeShape
public typealias GridCoord = AsciiTypes.GridCoord
public typealias DrawingCoord = AsciiTypes.DrawingCoord
public typealias Direction = AsciiTypes.Direction
public let Up = AsciiTypes.Up
public let Down = AsciiTypes.Down
public let Left = AsciiTypes.Left
public let Right = AsciiTypes.Right
public let UpperRight = AsciiTypes.UpperRight
public let UpperLeft = AsciiTypes.UpperLeft
public let LowerRight = AsciiTypes.LowerRight
public let LowerLeft = AsciiTypes.LowerLeft
public let Middle = AsciiTypes.Middle
public let ALL_DIRECTIONS = AsciiTypes.ALL_DIRECTIONS
public typealias Canvas = AsciiTypes.Canvas
public typealias AsciiStyleClass = AsciiTypes.AsciiStyleClass
public typealias AsciiEdgeStyle = AsciiTypes.AsciiEdgeStyle
public typealias AsciiNode = AsciiTypes.AsciiNode
public typealias AsciiEdge = AsciiTypes.AsciiEdge
public typealias AsciiSubgraph = AsciiTypes.AsciiSubgraph
public typealias AsciiConfig = AsciiTypes.AsciiConfig
public typealias AsciiGraph = AsciiTypes.AsciiGraph
public typealias CharRole = AsciiTypes.CharRole
public typealias RoleCanvas = [[CharRole?]]
public typealias AsciiTheme = AsciiTypes.AsciiTheme
public typealias ColorMode = AsciiTypes.ColorMode
public typealias EdgeBundle = AsciiTypes.EdgeBundle
...
public func mergeCanvasArray(_ base: Canvas, _ offset: DrawingCoord, _ useAscii: Bool, _ overlays: [Canvas]) -> Canvas {
    ...
    // Byte-identical to folding `mergeCanvases` over the overlays, but allocates
    // the accumulator ONCE and copies the base ONCE. The old fold reallocated a
    // full W×H canvas and recopied the whole base on *every* overlay — the
    // O(edges · W · H) hot-spot behind super-linear ASCII rendering. The final
    // size equals `max(base, every overlay + offset)`, which is exactly the size
    // the sequential fold converges to, and each overlay is applied in the same
    // order with the same per-cell rule, so the output is unchanged.
    var maxX = max(0, base.count - 1)
    var maxY = max(0, (base.first?.count ?? 1) - 1)
    for overlay in overlays {
        let oX = max(0, overlay.count - 1)
    ...
    let mergedW = merged.count
    let mergedH = merged.first?.count ?? 0

    for x in 0 ..< min(base.count, mergedW) {
        let col = base[x]
        let n = min(col.count, mergedH)
        for y in 0 ..< n {
            merged[x][y] = col[y]
        }
    }
    ...
    for overlay in overlays {
        for x in 0 ..< overlay.count {
            let mx = x + offset.x
            if mx < 0 || mx >= mergedW { continue }
            let col = overlay[x]
            for y in 0 ..< col.count {
                let c = col[y]
                if c == " " { continue }
                let my = y + offset.y
                if my < 0 || my >= mergedH { continue }
