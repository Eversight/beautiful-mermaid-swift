...
private func _bmParseMermaid(_ text: String) throws -> MermaidGraph {
    try parseMermaid(text)
}

private func _bmConvertToAsciiGraph(
    _ parsed: AsciiIndex.ParsedMermaid,
    _ config: AsciiIndex.AsciiConfig
) -> AsciiConverter.AsciiGraph {
    let converterConfig = AsciiConverter.AsciiConfig(
    ...
    // Bridge parser output (CoreTypes.MermaidGraph) to converter input
    guard let tsGraph = parsed.graph.payload as? CoreTypes.MermaidGraph else {
        // Fallback: empty graph if payload isn't the expected type
        return AsciiConverter.convertToAsciiGraph(
            AsciiConverter.MermaidGraphInput(
    ...
    let nodes: [(id: String, node: AsciiConverter.MermaidNodeInput)] = tsGraph.nodesInOrder.map { entry in
        (
            id: entry.id,
            node: AsciiConverter.MermaidNodeInput(
    ...
    let edges: [AsciiConverter.MermaidEdgeInput] = tsGraph.edges.map { e in
        AsciiConverter.MermaidEdgeInput(
            source: e.source,
            target: e.target,
            label: e.label,
    ...
    func convertSubgraph(_ sg: CoreTypes.MermaidSubgraph) -> AsciiConverter.MermaidSubgraphInput {
        let children = sg.children.map { convertSubgraph($0) }
        return AsciiConverter.MermaidSubgraphInput(
            label: sg.label,
            nodeIds: sg.nodeIds,
            children: children,
    ...
    let converterInput = AsciiConverter.MermaidGraphInput(
        nodes: nodes,
        edges: edges,
        subgraphs: subgraphs,
    ...
    return AsciiConverter.convertToAsciiGraph(converterInput, converterConfig)
}

private func _bmCreateMapping(_ graph: AsciiConverter.AsciiGraph) throws {
    try AsciiGrid.createMapping(graph)
}

private func _bmToAsciiTypeGraph(_ graph: AsciiConverter.AsciiGraph) -> AsciiGraph {
    ...
    let grid: [GridCoord: AsciiNode] = Dictionary(graph.grid.compactMap { key, node -> (GridCoord, AsciiNode)? in
...
public enum AsciiIndex {
    ...
    private struct AsciiGraphModel {
        var flowGraph: AsciiConverter.AsciiGraph
    ...
    private static func _mapTheme(_ theme: AsciiTheme, includeAccentBg: Bool = false) -> AsciiTypes.AsciiTheme {
        AsciiTypes.AsciiTheme(
    ...
    public static func renderMermaidASCII(
        ...
        case .xychart:
            ...
            let mappedConfig = AsciiTypes.AsciiConfig(
                useAscii: config.useAscii,
                paddingX: config.paddingX,
                paddingY: config.paddingY,
                boxBorderPadding: config.boxBorderPadding,
