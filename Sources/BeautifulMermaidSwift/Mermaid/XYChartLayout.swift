...
private func _layoutVertical(_ chart: XYChart) -> PositionedXYChart {
    ...
    let maxYLabelWidth = max(
        yTicks.map({ CoreStyles.estimateTextWidth(_formatTickValue($0), XY.axisLabelFontSize, XY.axisLabelFontWeight) }).max() ?? 0,
        XY.yLabelWidth
    )
...
private func _layoutHorizontal(_ chart: XYChart) -> PositionedXYChart {
    ...
    let maxCatLabelWidth = max(
        catLabels.map({ CoreStyles.estimateTextWidth($0, XY.axisLabelFontSize, XY.axisLabelFontWeight) }).max() ?? 0,
        40
    )
...
private func _buildLegendItems(_ chart: XYChart, _ centerX: Double, _ y: Double, _ colorMap: [Int]) -> [XYLegendItem] {
    ...
    let itemWidths = items.map { item in
        CoreStyles.estimateTextWidth(item.label, XY.legendFontSize, XY.legendFontWeight) + XY.legendSwatchW + XY.legendGap
    }
